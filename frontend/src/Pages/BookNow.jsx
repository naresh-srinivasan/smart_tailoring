...
//     const materialPrice = material && materialCosts[material]
//       ? parseFloat(materialCosts[material]) * parseFloat(materialNeeded || 0)
//       : 0;

//     const newTotal = base + extrasSum + materialPrice + DELIVERY_CHARGE + LABOUR_CHARGE;
//     setTotalCost(discount > 0 ? newTotal - newTotal * (discount / 100) : newTotal);
//   }, [materialNeeded, material, dressType, gender, formData, discount]);

//   // -------- UI helpers ----------
//   const toggleInstruction = (name) => setShowInstructions(prev => ({ ...prev, [name]: !prev[name] }));
...
//     const missing = measurements.filter(m => !formData[m.name] || String(formData[m.name]).trim() === "");
//     if (missing.length > 0) return alert(`Please fill all measurements: ${missing.map(m => m.name).join(", ")}`);
//     setStep(5);
//   };
...
//       );
//       if (!inventoryItem) return alert("Material/color not available");
//       if (materialNeeded > inventoryItem.total_quantity)
//         return alert(`Insufficient stock. Required: ${materialNeeded}m, Available: ${inventoryItem.total_quantity}m`);
...
//           buttons: { type: buttonType || undefined, color: buttonColor || undefined },
//         },
//         material,
//         color,
...
//         method: "PATCH",
//         headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
//         body: JSON.stringify({ total_quantity: inventoryItem.total_quantity - materialNeeded }),
//       });
...
//   const resetForm = () => {
//     setStep(1);
//     setGender(""); setDressType(""); setFormData({}); setShowInstructions({});
//     setMaterial(""); setColor(""); setCollarType(""); setSleeveType("");
//     setButtonType(""); setButtonColor(""); setMeasurementUnit("cm");
//     setMaterialNeeded(0); setTotalCost(0); setDeliveryAddress("");
//     setExpectedDate(""); setUseProfileAddress(false);
//     setShowCollarInfo(false); setShowSleeveInfo(false);
//     setOrderCompleted(false); setSubmittedOrder(null);
//     setShowConfetti(false); setIsSubmitting(false);
//     setPromoCode(""); setPromoError(""); setDiscount(0);
//   };

//   const stepTitles = useMemo(() => ["Gender", "Dress Type", "Style & Options", "Measurements", "Summary"], []);

//   if (orderCompleted && submittedOrder)
//     return <OrderCompleted showConfetti={showConfetti} submittedOrder={submittedOrder} resetForm={resetForm} />;

//   return (
//     <div className="min-h-screen bg-gray-50 py-10 px-6">
//       <motion.div className="max-w-5xl mx-auto bg-white rounded-3xl p-10 shadow-2xl"
...
//         {step === 2 && <DressTypeSelection gender={gender} onBack={() => setStep(1)} onSelect={handleDressTypeSelect} measurementsData={measurementsData} />}
//         {step === 3 && <StyleAndMaterialStep gender={gender} dressType={dressType} measurementsData={measurementsData}
//           material={material} setMaterial={setMaterial} color={color} setColor={setColor}
//           collarType={collarType} setCollarType={setCollarType} sleeveType={sleeveType} setSleeveType={setSleeveType}
//           buttonType={buttonType} setButtonType={setButtonType} buttonColor={buttonColor} setButtonColor={setButtonColor}
//           showCollarInfo={showCollarInfo} setShowCollarInfo={setShowCollarInfo}
//           showSleeveInfo={showSleeveInfo} setShowSleeveInfo={setShowSleeveInfo}
//           onBack={() => setStep(2)} onNext={proceedFromStyleToMeasurements} />}
//         {step === 4 && <MeasurementsForm gender={gender} dressType={dressType} measurementsData={measurementsData}
//           measurementUnit={measurementUnit} setMeasurementUnit={setMeasurementUnit}
//           formData={formData} setFormData={setFormData} showInstructions={showInstructions} toggleInstruction={toggleInstruction}
//           handleExtraChange={handleExtraChange} materialNeeded={materialNeeded} materialCosts={materialCosts} material={material}
//           totalCost={totalCost} currency={currency} setCurrency={setCurrency} conversionRate={conversionRates[currency] || 1}
//           DELIVERY_CHARGE={DELIVERY_CHARGE} LABOUR_CHARGE={LABOUR_CHARGE} onBack={() => setStep(3)} onSubmit={proceedToSummary} />}
//         {step === 5 && <OrderSummary gender={gender} dressType={dressType} material={material} color={color}
//           measurementUnit={measurementUnit} materialNeeded={materialNeeded} collarType={collarType} sleeveType={sleeveType}
//           buttonType={buttonType} buttonColor={buttonColor} formData={formData} measurementsData={measurementsData}
//           promoCode={promoCode} setPromoCode={setPromoCode} promoError={promoError} discount={discount}
//           applyPromoCode={applyPromoCode} removePromoCode={removePromoCode} totalCost={totalCost} userProfile={userProfile}
//           useProfileAddress={useProfileAddress} setUseProfileAddress={setUseProfileAddress} deliveryAddress={deliveryAddress}
//           setDeliveryAddress={setDeliveryAddress} expectedDate={expectedDate} setExpectedDate={setExpectedDate}
//           currency={currency} setCurrency={setCurrency} conversionRate={conversionRates[currency] || 1}
//           isSubmitting={isSubmitting} onBack={() => setStep(4)} onSubmit={handleFinalSubmit} />}
//       </motion.div>
//     </div>
//   );
// }
...
export default function BookNow() {
  ...
  useEffect(() => {
    const token = localStorage.getItem("token");
    if (!token) {
      window.location.href = "/login"; // 🚨 still using redirect, since router hook not injected here
      return;
    }
    fetch(`${API_BASE}/user/profile`, {
      headers: { Authorization: `Bearer ${token}` },
  ...
  useEffect(() => {
    ...
    const newTotal =
      base + extrasSum + materialPrice + DELIVERY_CHARGE + LABOUR_CHARGE;

    setTotalCost(
      discount > 0 ? newTotal - newTotal * (discount / 100) : newTotal
    );
  }, [materialNeeded, material, dressType, gender, formData, discount]);
  ...
  const proceedToSummary = (e) => {
    e.preventDefault();
    const measurements =
      measurementsData[gender]?.[dressType]?.measurements || [];
    const missing = measurements.filter(
      (m) =>
        !formData[m.name] || String(formData[m.name]).trim() === ""
    );
    if (missing.length > 0)
      return alert(
        `Please fill all measurements: ${missing
          .map((m) => m.name)
          .join(", ")}`
      );
    setStep(5);
